...
import { Player, FloatingText as FloatingTextType } from '../types';
import { BOARD_ROWS, BOARD_COLS, SNAKES, LADDERS } from '../constants';
...
const Board: React.FC<BoardProps> = ({ players, floatingTexts }) => {
  ...
  const renderSquares = () => {
    ...
    for (let row = BOARD_ROWS - 1; row >= 0; row--) {
      for (let col = 0; col < BOARD_COLS; col++) {
        ...
        const bgColor = isEven ? 'bg-teal-100' : 'bg-teal-200';
        
        squares.push(
          <div
            key={squareNumber}
            ref={el => { squareRefs.current[squareNumber - 1] = el; }}
            className={`relative w-full aspect-square flex items-center justify-center ${bgColor} text-gray-700`}
          >
            <span className="font-semibold text-xs md:text-sm">{squareNumber}</span>
             {SNAKES[squareNumber] && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4/5 h-4/5 opacity-40"><SnakeIcon /></div>}
            {LADDERS[squareNumber] && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4/5 h-4/5 opacity-40"><LadderIcon /></div>}
          </div>
        );
      }
    }
    ...
    return Object.entries(connections).map(([start, end]) => {
      ...
      return (
        <path
          ...
          fill="none"
          strokeLinecap="round"
          markerEnd={type === 'snake' ? `url(#arrowhead-${type})` : ''}
  ...
  return (
    <div ref={boardRef} className="relative aspect-square w-full bg-white rounded-lg shadow-2xl p-2 md:p-4 overflow-hidden border-4 border-teal-300">
      <div className="grid grid-cols-10 grid-rows-10 w-full h-full">
        {renderSquares()}
      </div>
      <svg className="absolute top-0 left-0 w-full h-full pointer-events-none">
        <defs>
            <marker id="arrowhead-snake" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
                <polygon points="0 0, 10 3.5, 0 7" fill="#dc2626" />
            </marker>
        </defs>
        {renderConnections(SNAKES, '#ef4444', 'snake')}
        {renderConnections(LADDERS, '#22c55e', 'ladder')}
      </svg>
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
        {players.map((player, index) => {
          const pos = squarePositions[player.position];
          if (!pos) return null;
          return <PlayerToken key={player.id} player={player} position={pos} playerIndex={index} totalPlayers={players.length}/>;
        })}
      </div>
